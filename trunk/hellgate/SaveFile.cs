...
namespace Hellgate
{
    ...
    public class SaveFile
    {
        ...
        private bool ReadUnit(SaveFile unit)
        {
            ...
            {
                Console.WriteLine("Error! Invalid Unit Major Version: " + unit._majorVersion.ToString("X4"));
                return false;
            }
            unit._minorVersion = _bitBuffer.ReadBits(8);
            if (unit._minorVersion != 0x00)
            {
                Console.WriteLine("Warning! Untested Unit Minor Version: " + unit._minorVersion.ToString("X2"));
            }
